<template>
  <div>
      ...
      <Row>
        <div class="i-box clearfix">
          ...
          <vxe-table :data="list" stripe show-overflow resizable @sort-change="sortChange">
            <vxe-table-column min-width="160" sortable field="whCode" :title="$t('lbl_wh_code')">
              <template v-slot="{ row }">
                <span class="pointer" @click="show(row.whCode, 'E')">{{ row.whCode }}</span>
              </template>
            </vxe-table-column>
            <vxe-table-column min-width="200" sortable field="whName" :title="$t('col_wh_name')"></vxe-table-column>
            <vxe-table-column min-width="180" sortable field="whAddress" :title="$t('col_warehouse_address')"></vxe-table-column>
    ...
    <!--编辑抽屉-->
    <look :drawer.sync="isLook" :flag="flag" :whCode="whCode" @ok-drawer="okDrawer" />
  </div>
</template>
  ...
  data () {
    return {
      ...
      head: {
        ...
        condition: this.$store.getters.UserCropCode     //2021-10-06 仓库代码   新加赋值语句
      },
      isLook: false
    }
  },
