 ...
 * @LastEditTime: 2020-05-31 22:56:18
 * @Description: PInv Count
 -->
<template>
      ...
      <Row>
        ...
        <vxe-table :data="list" max-height="600" stripe show-overflow resizable @sort-change="sortChange">
          <vxe-table-column min-width="160" sortable field="stockTakeCode" :title="$t('col_stock_take_code')">
            <template v-slot="{ row }">
              <router-link :to="`inv_count_detail/${row.stockTakeCode}?in=0&receiptStatus=${row.receiptStatus}`">{{ row.stockTakeCode }}</router-link>
            </template>
          </vxe-table-column>
...
export default {
  ...
  methods: {
    async getData () {
      var sendData = Object.assign(this.head, this.formData, { wfStatus: 1, stockTakeUser: 1, totalCount: -1, statusList: this.selectedData })
