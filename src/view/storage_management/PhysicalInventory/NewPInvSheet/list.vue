 ...
 * @LastEditTime: 2020-05-31 22:56:04
 * @Description: New PInv Sheet
 -->
<template>
    ...
    <div class="i-box clearfix">
      <Row>
        ...
        <orderSearch mode="checkbox" :selected.sync="selectedData" :data="checkDatasAll" @on-enter="searchHandle" @on-search="advancedHandle" @on-change="changeHandle">
          <Button class="ivu-btn-button-create" to="new_inv_sheet_detail?in=1&receiptStatus=0">{{ $t('sta_1') }}</Button>
        </orderSearch>
        </Col>
        <Col span="24">
        <vxe-table :data="list" max-height="600" stripe show-overflow resizable @sort-change="sortChange">
          <vxe-table-column min-width="160" sortable field="stockTakeCode" :title="$t('col_stock_take_code')">
            <template v-slot="{ row }">
              <router-link :to="`new_inv_sheet_detail/${row.stockTakeCode}?in=0&receiptStatus=${row.receiptStatus}`">{{ row.stockTakeCode }}</router-link>
            </template>
          </vxe-table-column>
          <vxe-table-column min-width="200" sortable field="docNum" :title="$t('col_sap_doc_code')"></vxe-table-column>
          ...
          <vxe-table-column min-width="180" sortable field="createUserName" :title="$t('col_creator_name')">
            <template v-slot="{ row }">
                <span>{{ row.createUserName == "ipasadmininstock123" ? 'SAP' :  row.createUserName  }}</span>
            </template>
          </vxe-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      checkDatasAll: [
        { value: 10, label: this.$t("sta_10") },
        { value: 45, label: this.$t('sta_45') },
        // { value: 50, label: this.$t('sta_50') },
        //{ value: 90, label: this.$t("sta_90") }
      ], // 备选项
      //selectedData: [10, 45, 50, 90], // 默认选中
      selectedData: [10, 45], // 默认选中
      list: [], // 表格数据
