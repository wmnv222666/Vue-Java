      ...
      <vxe-table ref="sTable" max-height="507" :sync-resize="tabVisible" :data="completeList" stripe show-overflow resizable :checkbox-config="{ reserve: true, highlight: true }" :tree-config="{ children: 'children' }" @checkbox-change="selectionChange" @checkbox-all="selectionChange">
        ...
        <vxe-table-column min-width="250" field="id" :title="$t('DNumber')" tree-node>
          <template slot-scope="scope">
            <a :href="'put_away_create?id='+scope.row.id" target="_Parent" class="buttonText">{{scope.row.id}}</a>
        ...
        <vxe-table-column min-width="100" field="showStatus" :title="$t('Status')">
          <!-- <template slot-scope="scope">
            <span v-if="scope.row.status==scope.row.status1">Not Started</span>
            <span v-if="scope.row.status>scope.row.status1">Partial Submited</span>
          </template> -->
        </vxe-table-column>
  ...
  methods: {
    ...
    async getDCList (receiptType) {
      ...
      for (let i = 0; i < res.data.list.length; i++) {
        if( res.data.list[i].status == res.data.list[i].status1){
          res.data.list[i].showStatus = "Not Started"  
        }else if(res.data.list[i].status > res.data.list[i].status1) {
          res.data.list[i].showStatus = "Partial Submited"
        }
      }
