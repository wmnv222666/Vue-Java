    ...
    <div class="i-box clearfix h100 x-height-table">
      <x-search-header @on-search="enterSearch">
        ...
        <template slot="body">
          ...
          <search-in flex v-show="groupBtn.buttonSubmit" :title="$t('ReceiveDate')" required>
        ...
        <Tabs v-model="tabVisible" class="h100" @on-click="tabClickHandle">
          <TabPane :label="p1Label" name="p1">
            ...
            <vxe-table-column min-width="200" field="shippingRef" :title="$t('ShippingRef')"></vxe-table-column> -->
                ...
                <template v-slot="{ row }">
                  <el-input-number  v-model="row.actualWeightQty" :min="1"></el-input-number>
                </template>
              </vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    async getDCList(receiptType) {
      ...
      // console.log('corpCode=',this.$store.getters.UserCropCode)
      var [err, res] = await this.$to(
    ...
    async saveOrder() {
      ...
      //-------------------
      //验证输入的 qty 是否 小于 1 + 10%
      let receiptQty = 0 ;
      let actualWeightQty = 0 ;

      this.selectionTodo.forEach( item => {
        actualWeightQty += item.actualWeightQty ;
      ...
      if (actualWeightQty > (receiptQty * 1.1)) {
        this.$Notice.warning({ desc: this.$t("msg_notice_355") });
        return false ;
      }
      //-----------------------
      ...
      /**  保留
      let selectParent = this.selectionTodo.filter( seleto => { return seleto.groupID == undefined  }) ;
      ...
      if(selectParent.length > 0) {
        for(i=0; i<selectParent.length; i++){
            ...
            this.selectionTodo.forEach( item => {
              if(item.groupID == selectParent[i]._XID || item._XID == selectParent[i]._XID){
                receiptQty = item.receiptQty ;
                actualWeightQty += item.actualWeightQty ;
              }
            })

            console.log("actualWeightQty : ", actualWeightQty)
            console.log("receiptQty : ", receiptQty * 1.1)
    ...
    preSaveValidation() {
      ...
      this.todoList.forEach( (el, index) => {
        let tmp = _.findIndex(this.checkSet, function(o) { return o == el._XID });
        if(tmp >= 0) {
          this.selectionTodo.push(this.todoList[index]);
        }
      })
    ...
    // 表格选中改变
    selectionChangeT({checked, selection }) {
      // this.selectionTodo = selection;
      console.log("selectionTodo", this.selectionTodo);
      ...
      console.log("sxid : ", sxid);

      if(sxid.length == this.todoList.length || sxid.length == 0) {
        this.checkSet = sxid ;
        return false ;
      }
      ...
      if ( checked ) {   //如果是 true,说明只有新元素被勾选, fasle 说明有元素被取消勾选
        let addXid = _.difference(sxid, this.checkSet) ; //找出新勾选的 _XID
        let addGID = '' ;
        this.todoList.forEach ( element => {
          if ( element._XID == addXid[0]) { addGID = element.groupID == undefined ? element._XID : element.groupID ; }
        });

        this.todoList.forEach ( (el, index) => {
          if ( addGID == el.groupID || addGID == el._XID) {
            arr.push( this.todoList[index] ) ;
            this.checkSet.push(el._XID) ;
          }
        });
        this.$refs.tTable.setCheckboxRow(arr, true);
        console.log("addGID : ", addGID);

      } else {
        let lessXid = _.difference(this.checkSet, sxid) ;  //找出取消勾选的 _XID
        let lessGID = '' ;
        this.todoList.forEach ( element => {
          if ( element._XID == lessXid[0]) { lessGID = element.groupID == undefined ? element._XID : element.groupID ; }
        });

        this.todoList.forEach ( (el, index) => {
          if ( lessGID == el.groupID || lessGID == el._XID) {
            arr.push( this.todoList[index] ) ;
            this.checkSet = this.checkSet.filter( ele => {
              return ele != el._XID ;
            });
          }
        });
        this.$refs.tTable.setCheckboxRow(arr, false);
        console.log("lessGID : ", lessGID);
      }

      console.log("sxid : ", sxid);
      console.log("arr : ", arr);
      console.log("change checkSet : ", this.checkSet);
    ...
    addRow(row, scope) {
      ...
      // console.log("addRow before", this.todoList)
      ...
      // console.log("addRow after", this.todoList)

      let selectRecords = this.$refs.tTable.getCheckboxRecords() ;
      selectRecords.forEach( el => {
          if (el._XID == b.groupID) {
            this.$refs.tTable.setCheckboxRow([this.todoList[scope.rowIndex + 1]], true);
            setTimeout(() => {
              this.checkSet.push(this.todoList[scope.rowIndex + 1]._XID);
              console.log("add checkSet : ", this.checkSet);
            }, 500);
      ...
      // let selectRecords = this.$refs.tTable.getCheckboxRecords() ;
      // let present = _.findIndex(selectRecords, x => { x._XID == b.groupID}) ;
      // if(present >= 0){
      //   this.$refs.tTable.setCheckboxRow([this.todoList[rowKey]], true);
      //   this.checkSet.push(this.todoList[scope.rowIndex + 1]._XID);
      // }
    ...
    removeRow(row) {
      ...
      _.pull(this.checkSet, row._XID) ;
      console.log("remove checkSet : ", this.checkSet);
