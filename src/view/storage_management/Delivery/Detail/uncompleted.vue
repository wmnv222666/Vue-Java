  ...
  <div class="modal-body padding-bottom-fixed">
    <div class="i-box clearfix">
      ...
      <vxe-table ref="sTable" max-height="507" :sync-resize="tabVisible" :data="completeList" stripe show-overflow resizable :checkbox-config="{ reserve: true, highlight: true }" :tree-config="{ children: 'children' }" @checkbox-change="selectionChange" @checkbox-all="selectionChange">
        ...
        <vxe-table-column min-width="200" field="id" :title="$t('DNumber')" tree-node>
          <template slot-scope="scope">
            <a :href="'delivery_create/'+scope.row.id+'/'+scope.row.receiptType" target="_Parent" class="buttonText">{{scope.row.id}}</a>
        ...
        <vxe-table-column min-width="100" field="s" :title="$t('Status')">
              <template v-slot="{ row }">
                <span v-if="row.s >= 20">Partial Saved</span>
                <span v-if="row.s < 20">Not Started</span>
              </template>
            </vxe-table-column>
        <vxe-table-column min-width="250" field="receiptType" :title="$t('DocumentType')">
          <template slot-scope="scope">
            <span v-if="scope.row.receiptType==201">Sales Docment</span>
            <span v-if="scope.row.receiptType==202">Transfer Out</span>
            <span v-if="scope.row.receiptType==203">Adjustment Out</span>
          </template>
        </vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    async getDCList (receiptType) {
      ...
      sendData.receiptStatusList=[90]
      sendData.receiptTypeString="201,202,203"
      //console.log("sendData",sendData)
    ...
    async getData () {
      ...
      //console.log("res=",res)
      this.head.totalCount = res.data.totalCount;
      //console.log(res.data.resultMap.todoList)
      this.completeList = this.dataSort(res.data.resultMap.todoList) || [];
    },
