  ...
  <div class="modal-body no-padding h100">
    <div class="i-box clearfix h100 x-height-table">
      ...
      <vxe-table :data="list" height="auto" stripe show-overflow resizable ref="sTable">
        ...
        <vxe-table-column min-width="160" field="weightQty" :title="$t('Diff Qty')"></vxe-table-column>
        ...
        <!-- <vxe-table-column min-width="160" field="stockTakeUser" :title="$t('col_stock_take_user')"></vxe-table-column> -->
      ...
      <vxe-table v-show="false" :data="list" height="auto" stripe show-overflow resizable ref="exportTable">
        <vxe-table-column min-width="160" field="matCode" :title="$t('Material*')"></vxe-table-column>
        <vxe-table-column min-width="160" field="stockTakeQty" :title="$t('Quantity*')"></vxe-table-column>
        <vxe-table-column min-width="160" field="stockType" :title="$t('Stock Type*')"></vxe-table-column>
        <vxe-table-column min-width="160" field="weightUnitCode" :title="$t('UofM')"></vxe-table-column>
        <vxe-table-column min-width="160" field="zeroCount" :title="$t('Zero Count')"></vxe-table-column>
        <vxe-table-column min-width="160" field="ext0" :title="$t('Serial Number')"></vxe-table-column>
        </vxe-table-column>
      </vxe-table>
  ...
  methods: {
    ...
    async getData () {
      ...
      for (var i = 0, l = this.list.length; i < l; ++i) {
        this.list[i].stockType=1
        if(this.list[i].stockTakeQty==0){
          this.list[i].zeroCount=0
        }else{
          this.list[i].zeroCount=''
        }
      }
    ...
    async exportExcel () {
      ...
      this.$refs.exportTable.exportData({
        filename: 'Count Diff Results',
        type: 'csv',
        isHeader: true,
        isFooter: true,
        data: this.list
